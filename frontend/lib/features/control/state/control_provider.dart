...
class ControlProvider extends ChangeNotifier {
  ...
  /// Switches the active shelf and loads its current device state from the API.
  ...
  Future<void> load() async {
    final requestedShelfId = _shelfId;
    _loading = true;
    _error = null;
    notifyListeners();

    try {
      final current = await _api.getShelfCurrent(requestedShelfId);
      if (_shelfId != requestedShelfId) return;
      _current = current;
      _deviceState = current.deviceState;
    } catch (e) {
      if (_shelfId == requestedShelfId) {
        _error = e.toString();
      }
    } finally {
      if (_shelfId == requestedShelfId) {
